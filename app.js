...
const CONFIG = {
  ...
  prizes: [
    ...
    { label: "Скидка на электрику", short: "Свет", emoji: "✦", weight: 15 },
    { label: "Бесплатная консультация", short: "Совет", emoji: "✧", weight: 22 },
    { label: "Подарок к ремонту", short: "Подарок", emoji: "◆", weight: 14 },
    { label: "Скидка на сантехнику", short: "Вода", emoji: "◌", weight: 14 },
    { label: "Бесплатный замер", short: "Замер", emoji: "⌁", weight: 17 },
    { label: "Бонус на материалы", short: "Бонус", emoji: "✺", weight: 5 }
  ],
  sectorColors: [
...
const exitTiming = {
  duration: 760,
  easing: "cubic-bezier(0.64, -0.06, 0.74, 0.18)"
};
...
async function fakeLoad() {
  const steps = [
    { p: 18, t: "Загружаем подарки..." },
    { p: 42, t: "Полируем золотое колесо..." },
    { p: 71, t: "Готовим вашу попытку..." },
    { p: 100, t: "Готово" }
  ];
  ...
  for (const step of steps) {
    ...
    await wait(390 + Math.random() * 180);
  }

  await loaderScreen.animate(
    [
...
async function transitionPanel(fromPanel, toPanel) {
  fromPanel.style.display = "block";

  await fromPanel.animate(
    [
      { opacity: 1, transform: "translateY(0) scale(1)" },
  ...
  fromPanel.style.removeProperty("display");

  toPanel.classList.remove("hidden");

  await toPanel.animate(
    [
      { opacity: 0, transform: "translateY(118vh) scale(0.965)" },
...
function drawWheel() {
  ...
  const backGlow = ctx.createRadialGradient(0, 0, radius * 0.30, 0, 0, radius + 28);
  ...
  ctx.strokeStyle = "rgba(207,166,90,0.88)";
  ctx.lineWidth = 12;
  ctx.stroke();

  ctx.beginPath();
  ctx.arc(0, 0, radius - 12, 0, Math.PI * 2);
  ctx.strokeStyle = "rgba(255,240,184,0.32)";
  ctx.lineWidth = 2;
  ctx.stroke();
  ...
  for (let i = 0; i < CONFIG.prizes.length; i++) {
    ...
    // Decorative asset-like medal
    ctx.beginPath();
    ctx.arc(radius - 76, 0, 34, 0, Math.PI * 2);
    ctx.fillStyle = "rgba(10,6,5,0.30)";
    ctx.fill();

    ctx.beginPath();
    ctx.arc(radius - 76, 0, 30, 0, Math.PI * 2);
    const medalGradient = ctx.createRadialGradient(radius - 86, -10, 5, radius - 76, 0, 34);
    ...
    ctx.font = "950 26px Manrope, system-ui";
    ctx.textAlign = "center";
    ctx.textBaseline = "middle";
    ctx.fillText(prize.emoji, radius - 76, -2);

    ctx.textBaseline = "alphabetic";
    ctx.fillStyle = "#FFF6DD";
    ctx.shadowColor = "rgba(0,0,0,0.65)";
    ctx.shadowBlur = 6;
    ctx.font = "950 15px Manrope, system-ui";
    ctx.fillText(prize.short, radius - 76, 51);
    ctx.shadowBlur = 0;

    // Studs
    ctx.beginPath();
    ctx.arc(radius - 18, 0, 5, 0, Math.PI * 2);
    ctx.fillStyle = "#FFF0B8";
    ctx.fill();

    ctx.beginPath();
    ctx.arc(radius - 18, 0, 2.3, 0, Math.PI * 2);
...
async function spinWheel() {
  ...
  function animateAnticipation(now) {
    ...
    function animateSpin(frameNow) {
      ...
      setTimeout(async () => {
        await transitionPanel(wheelPanel, prizePanel);
        isSpinning = false;
      }, 420);
    }
...
claimBtn.addEventListener("click", async () => {
  await transitionPanel(prizePanel, formPanel);
...
sendBtn.addEventListener("click", async () => {
  ...
  // Пока это GitHub Pages demo без бота.
  // Для финального Telegram WebApp потом включим отправку:
  // if (tg) {
  ...
  await transitionPanel(formPanel, successPanel);
