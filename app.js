...
function validatePhone(phone) {
  ...
  if (!/^\+[\d\s()\-]+$/.test(clean)) {
    return { valid: false, message: "Допустимы только цифры, пробелы, скобки и дефис" };
  }
  ...
  const ccMatch = clean.match(/^\+(\d{1,3})/);
  if (!ccMatch) {
    return { valid: false, message: "Укажите код страны после +" };
  }
