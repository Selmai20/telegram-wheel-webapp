...
function drawPrizeLines(ctx, lines, x, y, scale = 1) {
  ...
  ctx.shadowBlur = 6;
  ctx.font = `${Math.round(13 * scale)}px Manrope, system-ui`;
  ctx.fontWeight = "900";
...
function drawWheel() {
  ...
  for (let i = 0; i < CONFIG.prizes.length; i++) {
    ...
    const medalX = radius - 100;
    const medalY = -8;

    ctx.beginPath();
    ctx.arc(medalX, medalY, 42, 0, Math.PI * 2);
    ctx.fillStyle = "rgba(10,6,5,0.30)";
    ctx.fill();

    ctx.beginPath();
    ctx.arc(medalX, medalY, 36, 0, Math.PI * 2);
    const medalGradient = ctx.createRadialGradient(medalX - 12, medalY - 12, 5, medalX, medalY, 40);
    ...
    drawPrizeIcon(ctx, prize.iconType, medalX, medalY, 1);
    drawPrizeLines(ctx, prize.lines, medalX, 56, 1);
